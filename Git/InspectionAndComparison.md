...
The `git log` command is used to see the commit logs. The commits are ordered by time of commit. The log shows the following info:
- commit hash
- which branch the commit was made on
- author of the commit name + email
- commit message

Log out put can be customized using a `--format` flag will be explained in details [later]().
...
`..` Operator allows you to see commit difference when used with `git log` command; For example if you want to see commits in your feature branch that have not been shipped to Production, you would use `git log Production..feature/new-cool-feature`

### Syntax

- `git log branchA..branchB` -> see commit in branchB and not in BranchA


## git show

`git show` command is used to see a specifics of a commit.
...
- `--stat`: which files have changed and number of lines changed in file
