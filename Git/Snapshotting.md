...
In Git, "`commit`" is the a saved version of the state/data in the repository. Newer commits lay on top of previous commits, creating a chain/tree of commits, showcasing a history of changes to the state/data in the repository.
...
- `-i` or `--interactive`: This flag provides a interactive menu to add modifications of working directory to the staging area. Further explained [here](#git-add-interactive-mode)
...
- `-a`: Commits all files staged or unstaged.
...
Let us see some more advance topic in Snapshotting.
...
A maybe useful flag for `git add` (interactive)
